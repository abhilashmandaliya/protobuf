...
bool ReportSuccess(absl::string_view test_name,
  ...
  absl::Status status = GetGlobalTestEnvironment().ReportSuccess(test_name);
  if (!status.ok()) {
    *result_listener << "\n" << status.message();
    return false;
  }
...
bool ReportFailure(absl::string_view test_name,
                   ...
                   testing::MatchResultListener* result_listener) {
  absl::Status status =
      GetGlobalTestEnvironment().ReportFailure(test_name, failure_message);
  if (!status.ok()) {
...
bool ReportSkip(absl::string_view test_name,
                testing::MatchResultListener* result_listener) {
  absl::Status status = GetGlobalTestEnvironment().ReportSkip(test_name);
  if (!status.ok()) {
    *result_listener << "\n" << status.message();
    return false;
  }
